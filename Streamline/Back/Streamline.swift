...
class Streamline: AppDelegate, GameLogicDelegate {
    ...
    func getBoard(with level: Int) -> BoardInfo? {
        if level > self.getNumLevels() {
            fatalError("Level IndexOutOfBoundException")
        }
        else {
            let nextLevelBoard: BoardInfo? = nil
            if let board = nextLevelBoard {
                return board
            }
            else {
                fatalError("Level IndexOutOfBoundException")
